...
int Line_Following_Control_RBTPOS(double velocity_input, double starting_point_x, double end_point_x, double starting_point_y, double end_point_y, bool _RBTPOS);
...
int Line_Following_Control(double velocity_input, double starting_point_x, double end_point_x, double starting_point_y, double end_point_y, int check_event){
    ...
    while(ros::ok() && goal_reached == false && stop_interrupt==false){
       ...
       // Robot_configuration = mocapTBot.item_XY_YAW_configuration();
        ...
        Robot_configuration = mocapTBot.item_XY_YAW_configuration_OFFSET(-1.745+M_PI+M_PI);
...
int Line_Following_Control_RBTPOS(double velocity_input, double starting_point_x, double end_point_x, double starting_point_y, double end_point_y, bool _RBTPOS){
    ...
    std::cout << "... Initilization ..." << std::endl;
    std::cout << "... Gain - k2: " << k2 << " k3: " << k3 << std::endl;
    timenowdouble = time(NULL)+3;
    while(time(NULL)<timenowdouble){
        ros::spinOnce();
        Robot_configuration = mocapTBot.item_XY_YAW_configuration_OFFSET(-1.745+M_PI);
        ros::Duration(0.01).sleep();
      }

    if(Robot_configuration[2]<1 && Robot_configuration[2]>-1){
        if(Robot_configuration[0]>end_point_x)  // CASE A
          invert_robot_config = true;
        else                                    // CASE C
          invert_robot_config = false;
      }
    else{
        if(Robot_configuration[0]>end_point_x)  // CASE B
          invert_robot_config = false;
        else
          invert_robot_config = true;           // CASE D
      }
    ...
    if(invert_robot_config==false){
        if(_RBTPOS==0){
          while(ros::ok && goal_reached==false){
            ros::spinOnce();
            Robot_configuration = mocapTBot.item_XY_YAW_configuration_OFFSET(-1.745+M_PI);
            std::cout << "sinistra vado su 0 - robot config: " << Robot_configuration[2] << std::endl;
            Robot.move_robot(0, M_PI/4);
            ros::Duration(0.01).sleep();
            if(Robot_configuration[2]<0.03 && Robot_configuration[2]>-0.03)
              goal_reached=true;
            }
          }
        else{
            while(ros::ok && goal_reached==false){
              ros::spinOnce();
              Robot_configuration = mocapTBot.item_XY_YAW_configuration_OFFSET(-1.745+M_PI);
              std::cout << "destra vado giu PI - robot config: " << Robot_configuration[2] << std::endl;
              Robot.move_robot(0, M_PI/4);
              if(Robot_configuration[2]>2.8 || Robot_configuration[2]<-2.8){
                Robot.move_robot(0, 0);
                Robot.move_robot(0, 0);
                Robot.move_robot(0, 0);
                goal_reached=true;
